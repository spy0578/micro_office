...
class BaseGetClass(MethodView):
    ...
    def get():
        ...
        charger_id = param_dict['charger_id']
        ...
        if charger_info.charger_stat == const.CHARGER_STAT_CHARGING and charger_info.charger_auth == const.AUTHORIZATION :
            ret_code = const.RET_STOPING_CHARGING
            desc = '结束充电准备中'
            ...
            if charging is None:
                return ret_func(const.RET_DB_ERROR, '充电表查询无数据', {})
            ...
            charging_h = db_session.query(TblChargingH).filter(TblChargingH.charger_id == charger_id,
                                                               TblChargingH.user_id    == user_id,
